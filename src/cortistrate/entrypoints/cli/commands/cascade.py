"""``cortistrate cascade`` subcommand group.
...
from cortistrate.infra.persistence.pg.pg_manager import init as pg_init
from cortistrate.infra.persistence.pg.pg_manager import dispose as pg_dispose
from cortistrate.infra.persistence.sqlite import (
    dispose_engine,
    get_engine,
    md_change_state_repo,
)
from cortistrate.memory.cascade import CascadeOrchestrator, match_kind
    ...
    async def _run() -> None:
        async with _runtime():
            ...
            if not apply:
                ...
                if retryable:
                    typer.echo(
                        f"run `cortistrate cascade fix --apply` to re-enqueue "
