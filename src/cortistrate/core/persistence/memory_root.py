...
The default location and tunables come from :class:`cortistrate.config.Settings`
(loaded from ``config/default.toml`` + ``CORTISTRATE_*`` environment variables);
...
class MemoryRoot:
    ...
    def default(cls, *, explicit_root: str | None = None) -> MemoryRoot:
        ...
        Resolution: ``explicit_root`` > ``CORTISTRATE_ROOT`` env > ``~/.cortistrate``.
    ...
    def postgres_dir(self) -> Path:
        """``<root>/.index/postgres/`` — Postgres table root."""
        return self.index_dir / "postgres"
    ...
    def sqlite_dir(self) -> Path:
        ...
        mode). Symmetric with :attr:`postgres_dir`.
        """
