...
ENV UV_PROJECT_ENVIRONMENT="/opt/cortistrate/venv"
ENV VIRTUAL_ENV="/opt/cortistrate/venv"
ENV PATH="/opt/cortistrate/venv/bin:$PATH"
...
LABEL maintainer="Cortistrate"
LABEL description="Cortistrate memory framework — PG 18 + pgvector + app"
...
# Create non-root user for the cortistrate app process.
...
COPY --from=builder /opt/cortistrate/venv /opt/cortistrate/venv
# Make venv world-readable/executable so the 'app' user can run it.
RUN chmod -R a+rX /opt/cortistrate/venv
ENV VIRTUAL_ENV="/opt/cortistrate/venv"
ENV PATH="/opt/cortistrate/venv/bin:$PATH"
...
RUN mkdir -p /home/app/.cortistrate/.index/pg \
             ...
             /home/app/.cortistrate/.tmp

# ── Environment defaults (overridable via docker run -e) ───────────
ENV CORTISTRATE_ROOT=/home/app/.cortistrate
ENV CORTISTRATE_API__HOST=0.0.0.0
ENV CORTISTRATE_API__PORT=5473
ENV DB_HOST=127.0.0.1
ENV DB_PORT=5432
ENV DB_NAME=cortistrate
ENV DB_USER=cortistrate
ENV DB_PASSWORD=cortistrate_local_2026
ENV PGDATA=/home/app/.cortistrate/.index/pg
ENV LC_ALL=C.UTF-8
ENV LANG=C.UTF-8
ENV TZ=UTC
