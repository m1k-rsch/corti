...
Two modes:
  --mode lifespan (default)   Full strict check through the cortistrate app
                              ...
                              KIND_REGISTRY.
  --mode readonly             Bypass the lifespan stack, open Postgres with
...
async def run_lifespan_mode(corpus: Path) -> int:
    ...
    async with app.router.lifespan_context(app):
        # 1+2. id set + sha
        print("─── md ↔ Postgres strict consistency ───")
        try:
            stats = await assert_md_pg_strict_consistent(corpus)
            print("  PASS")
        ...
        if stats is not None:
            ...
            print(
                ...
                f" {'md_entries':>12s} {'pg_rows':>12s}"
            )
            print("  " + "─" * 53)
            for kind, s in stats.items():
                print(
                    f"  {kind:<15s} {s.md_file_count:>10d}"
                    f" {s.md_entry_count:>12d} {s.pg_row_count:>12d}"
                )
...
async def run_readonly_mode(corpus: Path, owners_filter: list[str] | None) -> int:
    """Direct Postgres read + md read; no lifespan / cascade / ome started.
    ...
    print("─── md ↔ Postgres consistency (readonly) ───")
    rc = 0
    for table_name, dir_name, prefix, handler_cls in kinds:
        try:
            table = db.open_table(table_name)
        except FileNotFoundError:
            print(f"  {table_name}: table not in postgres (skip)")
            continue
        for owner in owners:
            ...
            only_md = sorted(set(md_sha_total) - set(pg_sha))
            only_pg = sorted(set(pg_sha) - set(md_sha_total))
            sha_mismatch = sorted(
                k
                for k in set(md_sha_total) & set(pg_sha)
                if md_sha_total[k] != pg_sha[k]
            )
            ok = not (only_md or only_pg or sha_mismatch)
            status = "OK" if ok else "DRIFT"
            if not ok:
                rc = 1
            print(
                f"  {table_name:<12s} owner={owner:<12s}"
                f" md={len(md_sha_total):5d} pg={len(pg_sha):5d}"
                f"  {status}"
            )
            if only_md:
                print(f"    only_in_md (first 5):    {only_md[:5]}")
            if only_pg:
                print(f"    only_in_pg (first 5): {only_pg[:5]}")
            if sha_mismatch:
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        "readonly = direct postgres read (safe on active corpus)",
    )
    p.add_argument(
        "--owners",
