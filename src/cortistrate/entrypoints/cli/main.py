"""cortistrate CLI root entry point.

Exposed as the ``cortistrate`` console script in ``pyproject.toml``. Subcommand
groups live under :mod:`cortistrate.entrypoints.cli.commands` and are registered
here.
...
app = typer.Typer(
    name="cortistrate",
    help="cortistrate — md-first memory extraction framework",
