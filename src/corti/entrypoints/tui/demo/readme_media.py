"""Render README media for the ``corti demo`` TUI."""
...
CORTI_TERMINAL_THEME = TerminalTheme(
    background=(29, 28, 24),
...
async def render_media(out_dir: Path) -> tuple[Path, Path]:
    ...
    screenshot = out_dir / "corti-demo-tui-screenshot.svg"
    ...
    animation = out_dir / "corti-demo-tui-animation.svg"
...
def _export_screenshot_svg(app) -> str:
    ...
    return console.export_svg(
        title=app.title,
        theme=CORTI_TERMINAL_THEME,
        unique_id="terminal-corti",
    )
...
def _build_animation_svg(frame_paths: Sequence[Path], plan: Sequence[FramePlan]) -> str:
    ...
    for index, frame_path in enumerate(frame_paths):
        ...
        images.append(
            ...
            f"    <title>Corti demo state: {state}</title>\n"
    ...
    return (
        ...
        "<title>Animated Corti demo terminal UI</title>\n"
        "<desc>Corti demo cycles through memory states with a closed loop.</desc>\n"
        + "\n".join(images)
        + "\n</svg>\n"
    )
...
def normalize_svg_terminal_ids(svg: str) -> str:
    ...
    return re.sub(r"terminal-\d+", "terminal-corti", svg)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/tmp/corti-demo-media"),
