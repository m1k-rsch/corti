...
Cortistrate follows a **two-zone discipline**:

* **Storage** (SQLite + Postgres) is always UTC. Use :func:`get_utc_now`
...
* **Display** (markdown frontmatter, HTTP API response, date buckets for
  ...
  :attr:`cortistrate.config.MemorySettings.timezone` (``CORTISTRATE_MEMORY__TIMEZONE``).
...
def _display_tz() -> _dt.tzinfo:
    ...
    Reads :attr:`cortistrate.config.MemorySettings.timezone`; that field
...
def get_utc_now() -> _dt.datetime:
    ...
    Postgres row construction, OME event ``ts``, any internal "when
    did this happen" record). Independent of the display timezone — a
    new deployment that switches ``CORTISTRATE_MEMORY__TIMEZONE`` will not
