...
Cortistrate uses a two-file TOML configuration system with environment variable
overrides for container deployments.

## Quick Start

```bash
# 1. Generate config files in the default root (~/.cortistrate/)
cortistrate init

# 2. Edit the config — fill in API keys
$EDITOR ~/.cortistrate/cortistrate.toml
...
### `cortistrate.toml` — Application Settings

Located at `<root>/cortistrate.toml`. Controls all application behavior: API
...
Generated by `cortistrate init` from the shipped `config/default.toml`
...
Generated by `cortistrate init` from the shipped `config/default_ome.toml`
...
2. `<root>/cortistrate.toml` — user configuration (optional)
3. `CORTISTRATE_*` environment variables — container/CI overrides
...
| Source | Example |
|---|---|
| `--root` CLI flag | `cortistrate server start --root /data/cortistrate` |
| `CORTISTRATE_ROOT` env var | `export CORTISTRATE_ROOT=/data/cortistrate` |
| Default | `~/.cortistrate` |

All CLI commands that interact with storage accept `--root`:

```bash
cortistrate server start --root /data/cortistrate
cortistrate cascade status --root /data/cortistrate
cortistrate config show --root /data/cortistrate
cortistrate init --root /data/cortistrate
```
...
| `index_cache_size_bytes` | int | `16777216` | Upper bound on Postgres index cache (16 MB default). |
...
Use `cortistrate config show` to inspect the effective configuration:

```bash
cortistrate config show
cortistrate config show --root /data/cortistrate
```
...
In containerized environments, skip `cortistrate init` and use environment
variables directly:

```dockerfile
ENV CORTISTRATE_ROOT=/data/cortistrate
ENV CORTISTRATE_LLM__API_KEY=sk-...
ENV CORTISTRATE_LLM__MODEL=gpt-4o
ENV CORTISTRATE_EMBEDDING__MODEL=text-embedding-3-large
ENV CORTISTRATE_EMBEDDING__API_KEY=sk-...
ENV CORTISTRATE_EMBEDDING__BASE_URL=https://api.openai.com/v1
ENV CORTISTRATE_API__HOST=0.0.0.0
ENV CORTISTRATE_API__PORT=8000
```

Environment variable naming convention:

```
CORTISTRATE_<SECTION>__<KEY>
```
...
|---|---|
| `[llm] api_key = "sk-..."` | `CORTISTRATE_LLM__API_KEY=sk-...` |
| `[sqlite] busy_timeout_ms = 10000` | `CORTISTRATE_SQLITE__BUSY_TIMEOUT_MS=10000` |
| `[memory] timezone = "Asia/Tokyo"` | `CORTISTRATE_MEMORY__TIMEZONE=Asia/Tokyo` |
