...
Memory uses Markdown as the single source of truth; SQLite and Postgres are
...
| Postgres | Vectors + BM25 index for Episodes / atomic facts | Search (rebuildable from Markdown) |
...
> Soft-archive, not delete: even if SQLite / Postgres are corrupted, as long
...
- [storage_layout.md](storage_layout.md) — Markdown + SQLite + Postgres stack
