...
from cortistrate.component.utils.datetime import from_timestamp, to_timestamp_ms
from cortistrate.core.observability.logging import get_logger
from cortistrate.infra.persistence.sqlite import cluster_repo
from cortistrate.memory.search.callbacks import build_rerank_fn
from cortistrate.memory.search.shaper import shape_episode_from_candidate
...
if TYPE_CHECKING:
    ...
    from cortistrate.memory.search.recall import AtomicFactRecaller, EpisodeRecaller
...
# len(full_fact_corpus); Cortistrate doesn't know the corpus size upfront,
# so we pass a large sentinel and let the Postgres limit clamp naturally.
...
async def search_episodes_agentic(
    ...
    *,
    ...
    Args:
        ...
        where: Pre-compiled Postgres filter string (owner + any request filters).
...
def _to_everalgo_doc_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    keys and the date from a ms-epoch ``metadata["timestamp"]``. cortistrate
