...
* ``owner_type`` is a hard partition. ``user`` returns ``episodes``
  (and optionally ``profiles``). The ``data.*`` arrays always exist; routes not
...
allow-list / safety validation runs in :mod:`cortistrate.memory.search.filters`
...
class FilterNode(BaseModel):
    ...
    safety is enforced when compiling the node to a Postgres ``where``
    string in :mod:`cortistrate.memory.search.filters`.
    """
...
class SearchRequest(BaseModel):
    ...
    """App / project scope (default ``\"default\"``). Pinned into the Postgres
