...
from corti.component.utils.datetime import from_timestamp, to_timestamp_ms
from corti.core.observability.logging import get_logger
from corti.infra.persistence.sqlite import cluster_repo
from corti.memory.search.callbacks import build_rerank_fn
from corti.memory.search.shaper import shape_episode_from_candidate
...
if TYPE_CHECKING:
    ...
    from corti.memory.search.recall import AtomicFactRecaller, EpisodeRecaller
...
# len(full_fact_corpus); Corti doesn't know the corpus size upfront,
...
def _to_everalgo_doc_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    keys and the date from a ms-epoch ``metadata["timestamp"]``. corti
