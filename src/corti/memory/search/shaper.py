...
* Simple kinds — :func:`shape_episode_from_candidate`.
  Each consumes one :class:`Candidate` and emits the matching ``SearchXxxItem``.
...
The shaper is pure - no DB calls. When fact eviction replaces a
...
def shape_atomic_fact_from_candidate(candidate: Candidate) -> SearchAtomicFactItem:
    """Cast a DB ``atomic_fact`` candidate row into the nested fact DTO."""
