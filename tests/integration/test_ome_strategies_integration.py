...
async def test_skill_chain_e2e(
    ...
    pipeline is all real. Only mocked: Postgres reads (case + skill),
    ...
    target_pg = MagicMock()
    target_pg.entry_id = "ac_20260517_0001"
    target_pg.timestamp = _dt.datetime(2026, 5, 17, tzinfo=_dt.UTC)
    target_pg.task_intent = "summarise the doc"
    target_pg.approach = "read + condense"
    target_pg.quality_score = 0.8
    target_pg.key_insight = ""
    ...
    with (
        ...
        mock_case_repo.find_by_owner_entry = AsyncMock(return_value=target_pg)
