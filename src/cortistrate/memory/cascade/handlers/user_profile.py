...
emissions (list of small dicts mixed with strings). Postgres has no
...
from cortistrate.infra.persistence.pg import UserProfile, user_profile_repo
...
class UserProfileHandler(Handler):
    ...
    kind = "user_profile"
    db_repo: ClassVar[Any] = user_profile_repo
    """Exposed for ``CascadeWorker._optimize_touched_kinds`` - the
    worker discovers the Postgres repo to optimize via this attribute,
