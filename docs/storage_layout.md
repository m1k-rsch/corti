...
│   └── (Postgres — external server; not under .index/)
│
...
> rather than `entry_id` alone — see SQLite/Postgres tables that follow.

## 5. SQLite + Postgres derived indexes
...
└── (Postgres — external server)
    └── per-kind tables     one table per business kind — the per-kind
...
Episode and AtomicFact Postgres tables carry a `deprecated_by: str | None`
