"""KindRecaller protocol + DB row -> Candidate helpers.
...
Both are filtered by the precompiled DB ``where`` string and capped
...
constructor signatures identical across the four DB-backed
...
# PostgreSQL tsvector query types were used by the original recallers.
# PG recallers use tsvector + plainto_tsquery and don't need these types.
# We keep them as ``object`` aliases so any legacy code that type-annotates
# against them doesn't break at import time.
FullTextQuery = object  # type: ignore[assignment,misc]
BooleanQuery = object  # type: ignore[assignment,misc]
MatchQuery = object  # type: ignore[assignment,misc]
Occur = object  # type: ignore[assignment,misc]

# Columns that should never travel through the ranker / shaper. ``vector``
# is huge (1024 floats); ``_distance`` belongs to the DB query engine
...
class RecallerDeps:
    """Shared dependencies for every DB-backed recaller.
...
def row_to_candidate(
    ...
    *,
    ...
    """Pack a DB row dict into an everalgo ``Candidate``.

    The full row (minus noise columns) rides in ``metadata`` so the
    shaper can build the response DTO without going back to the DB.
    """
    rid = row.get("id")
...
def cosine_score_from_distance(distance: float | None) -> float:
    """Convert PostgreSQL cosine ``_distance`` -> similarity in ``[0, 1]``.
...
def tokenize_query(tokenizer: Tokenizer, query: str) -> str:
    ...
    ``*_tokens`` columns; PostgreSQL tsvector expects a whitespace-tokenised
...
def build_or_query(
    ...
    - Clauses combine under :class:`BooleanQuery` with
      ...
      the document.
    - PostgreSQL still computes a proper joint BM25 score from all
