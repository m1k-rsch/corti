"""``corti demo`` — first-run memory sphere demo."""
...
LIVE_DEMO_SESSION_ID = "corti-demo-live"
LIVE_DEMO_USER_ID = "corti_demo_user"
...
def register(parent: typer.Typer) -> None:
    ...
    def demo(
        ...
        live: bool = typer.Option(
            False,
            "--live",
            help="Connect to a running Corti server and run add/flush/search.",
        ),
        server_url: str = typer.Option(
            LIVE_DEMO_SERVER_URL,
            "--server-url",
            help="Corti server URL used by --live.",
        ),
    ) -> None:
        """Launch the Corti first-memory Textual TUI."""
        if live:
            _run_live_demo(
                cinematic=cinematic,
...
def _load_run_demo_tui():
    try:
        from corti.entrypoints.tui.demo.app import run_demo_tui
    except ModuleNotFoundError as exc:
        if exc.name != "textual":
            raise
        typer.secho(
            "error: Textual is required for `corti demo`; install the "
            "package with TUI dependencies or run `corti demo --plain`.",
            fg=typer.colors.RED,
            err=True,
        )
...
def _run_live_demo_flow(
    ...
    *,
    ...
    """Run the educational demo story through a live Corti server."""
    ...
    if health.get("status") != "ok":
        raise typer.BadParameter(
            f"Corti server at {base_url} did not return healthy status"
        )
    ...
    raise typer.BadParameter(
        "Corti server accepted the memory, but search did not return it yet. "
        "Try `corti demo --live` again after indexing catches up."
    )
...
def _request_json(
    ...
    *,
    ...
    try:
        ...
        raise typer.BadParameter(
            f"Could not reach Corti server at {base_url}. "
            "Start it with `corti server start` and try again."
        ) from exc
    if not raw:
        return {}
    parsed = json.loads(raw)
    if not isinstance(parsed, dict):
        raise typer.BadParameter(f"Corti server returned non-object JSON: {url}")
    return parsed
...
def _print_plain_demo(story: DemoStory | None = None) -> None:
    ...
    console.print(
        Panel(
            ...
            title="Corti Memory Sphere",
            border_style=CORTI_YELLOW,
        )
    )
    console.print(f"[bold {CORTI_GREEN}]Corti remembered:[/]")
    console.print(story.memory)
    console.print()
    console.print(f"[bold {CORTI_YELLOW}]Source:[/] {story.source_filename}")
