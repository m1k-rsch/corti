...
The cascade daemon keeps Postgres in sync with the markdown files under
the memory root. Service / entry points only ever write markdown; the
daemon is the **sole** writer of the Postgres index. This runbook covers
...
4. **Postgres** — async connection + schema verification + FTS indexes.
...
| Worker | `claim_pending_batch` polling (default 1 s when idle) | Handler dispatch → Postgres upsert / delete |
...
### Postgres schema drift on startup

`PostgresLifespanProvider.startup` calls `verify_business_schemas`. If
...
the worker repopulates Postgres. Markdown is the source of truth, so
no data is lost.
...
- Postgres query / index build fails with `postgres error: ... Too many
  open files (os error 24)`.
- `lsof -p <pid> | wc -l` grows monotonically over hours / days.
- Health log lines like `cascade_postgres_optimize_failed` /
  `cascade_postgres_rebuild_failed` carrying `OSError: [Errno 24]`.

Cause (verified against `postgres 4.0`): the Postgres *index* cache
...
Postgres's own default (`index_cache_size_bytes=None`, unbounded), they
...
Fix (already wired in `PostgresSettings.index_cache_size_bytes` —
default 16 MB, ~290 FD ceiling): see
[Tuning knobs § Postgres index cache](#postgres-index-cache-index_cache_size_bytes)
...
at Postgres's default (unbounded) because the metadata cache holds
...
- **Schema migration**: Postgres column changes require `rm -rf`.
