...
``everalgo-parser`` is an optional dependency (``cortistrate[multimodal]``).
...
from cortistrate.core.errors import MultimodalNotEnabledError, UnsupportedModalityError
...
def require_parser() -> None:
    ...
    if not parser_available():
        raise MultimodalNotEnabledError(
            ...
            "Install with: pip install 'cortistrate[multimodal]'"
        )
...
async def aparse_file(raw_file: RawFile) -> ParsedContent:
    ...
    from cortistrate.component.llm import get_multimodal_llm_client
    from cortistrate.core.errors import LLMServiceError
