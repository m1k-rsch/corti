...
class TestKnowledgeKindRegistration:
    ...
    def test_knowledge_document_has_no_db_schema(self) -> None:
        spec = next(k for k in KIND_REGISTRY if k.name == "knowledge_document")
        assert spec.db_schema is None
        assert spec.db_repo is None

    def test_knowledge_topic_has_db_schema(self) -> None:
        spec = next(k for k in KIND_REGISTRY if k.name == "knowledge_topic")
        assert spec.db_schema is not None
        assert spec.db_repo is not None
