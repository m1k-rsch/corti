...
Pool settings are read from ``corti-pg.env`` or environment variables:
    - DB_HOST (default: localhost)
    - DB_PORT (default: 5432)
    - DB_NAME (default: corti)
    - DB_USER (default: corti)
    - DB_PASSWORD (default: corti_local_2026)
...
def _load_env() -> dict[str, str | int]:
    """Load PG connection params from corti-pg.env, falling back to env vars.

    Resolution order (first wins):
        1. ``corti-pg.env`` in project root (absolute path)
        2. ``CORTI_PG_ENV`` env var pointing to a .env file
        3. ``DB_HOST`` / ``DB_PORT`` / etc. environment variables
        4. Built-in defaults
    """
    params: dict[str, str | int] = {}

    # Try corti-pg.env from known location
    env_file = os.environ.get("CORTI_PG_ENV", "")
    if not env_file:
        candidates = [
            Path("/home/u1/.corti/pg.env"),
        ]
        for c in candidates:
            if c.exists():
                env_file = str(c)
