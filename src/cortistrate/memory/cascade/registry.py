...
``/`` separator — see ``17_postgres_tables_design.md`` §2.4.2 and
...
from cortistrate.core.persistence.markdown import BaseFrontmatter
...
class KindSpec:
    """One cascade kind - md schema + Postgres binding + handler factory.
    ...
    ``db_schema`` + ``db_repo`` describe the destination side.
...
KIND_REGISTRY: tuple[KindSpec, ...] = (
    ...
    KindSpec(
        ...
        db_schema=KnowledgeTopic,
        db_repo=knowledge_topic_repo,
    ),
)
