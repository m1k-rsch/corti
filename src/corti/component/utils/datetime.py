...
Corti follows a **two-zone discipline**:
...
* **Display** (markdown frontmatter, HTTP API response, date buckets for
  ...
  :attr:`corti.config.MemorySettings.timezone` (``CORTI_MEMORY__TIMEZONE``).
...
def _display_tz() -> _dt.tzinfo:
    ...
    Reads :attr:`corti.config.MemorySettings.timezone`; that field
...
def get_utc_now() -> _dt.datetime:
    ...
    new deployment that switches ``CORTI_MEMORY__TIMEZONE`` will not
