...
How Cortistrate turns a stream of messages into durable, searchable memory —
...
| **Postgres** (Arrow) | `.index/postgres/tables` | vector + BM25 + scalar columns for retrieval | ✅ from markdown |
...
The default memory root is **`~/.cortistrate/`** (override with
`CORTISTRATE_ROOT` env var or `--root` on the CLI). Configuration lives
inside the memory root as `cortistrate.toml` (generated by `cortistrate init`).
...
!!! warning "Differences from older PRD-era docs"
    ...
    scope). There is **no `cortistrate reindex` command** (see
    [Operating it](#operating-it)).

The path manager is
[`MemoryRoot`](../src/cortistrate/core/persistence/memory_root.py); every path
above is a property on it. `MemoryRoot.ensure()` creates the runtime dirs
(`.index/{sqlite,postgres}/`, `.tmp/`); user-visible dirs appear on first
write. Config files (`cortistrate.toml`, `ome.toml`) are created by `cortistrate init`.
...
- The **cascade daemon** turns every `.md` write into Postgres rows so the
...
The cascade subsystem keeps Postgres in sync with the markdown tree. It runs
...
with `cortistrate cascade` ([Operating it](#operating-it)); deeper runbook in
...
|---|---|---|
| **Write** (`/add`, `/flush`) | **strong** | the episode `.md` is on disk before the call returns `extracted`; never blocks on Postgres |
| **Read** (`/search`, `/get`) | **eventual** | reads Postgres, which lags md by the cascade processing time — sub-second typically, up to ~10–15 s under load |
...
Postgres** engine in one query; SQLite is a local file. The whole stack is a
...
|---|---|
| `cortistrate init` | generate starter config files (`cortistrate.toml` + `ome.toml`) |
| `cortistrate server start` | run the HTTP API (cascade + OME start with it) |
| `cortistrate cascade status` | queue / LSN summary |
| `cortistrate cascade sync` | drain the cascade queue now (force md → Postgres) |
| `cortistrate cascade fix` | list failed rows / re-enqueue retryable ones |

!!! warning "There is no `cortistrate reindex` or `cortistrate flush`"
    - **Reindex** = the index is rebuildable: stop the server,
      `rm -rf <memory-root>/.index/postgres`, restart — the cascade
