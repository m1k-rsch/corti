"""Real-Postgres tests for ``AtomicFactRecaller.facts_for_episodes``.
...
These tests exercise the real Postgres query path (no recaller stubs):
...
- empty / unknown parents → empty result, no Postgres call surprise.
"""
...
async def test_facts_for_episodes_buckets_by_shared_memcell() -> None:
    ...
    one Episode per user sender) gives multiple Postgres episode rows
