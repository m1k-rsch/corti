...
### Postgres tables — zero configuration

```python
import datetime as _dt

class MyPgRow(BasePgTable):
    ts: _dt.datetime   # automatically tz=UTC in the Arrow schema
```

Postgres's Pydantic → PyArrow converter does not understand
...
Instead, `BasePgTable.to_arrow_schema()` walks the inferred schema
