...
recipe: a single ``ProcessorFormatter`` renders both cortistrate's own
...
Rust-side loggers (Postgres / Arrow) live in the Rust ``log``
...
def configure_logging(level: str = "INFO") -> None:
    ...
    * Root-logger handlers are replaced with a single ``StreamHandler``
    ...
    # produces a thousand such lines, drowning cortistrate's own events. They
