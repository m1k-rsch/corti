...
:func:`cortistrate.core.persistence.markdown.entries._render_value` —
...
from cortistrate.core.persistence import app_id_from_dir, project_id_from_dir
...
def content_sha256(named_parts: dict[str, str]) -> str:
    ...
    don't propagate to Postgres and don't waste an embedding call.
    """
...
def resolve_scope(md_path: str) -> tuple[str, str]:
    ...
    ``"default"`` id (see :func:`cortistrate.core.persistence.app_id_from_dir`).
...
def parse_inline_list(raw: str) -> list[str]:
    ...
    Mirrors :func:`cortistrate.core.persistence.markdown.entries._render_value`
