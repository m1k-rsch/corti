"""Dot-sphere primitives for the Cortistrate demo TUI.
...
CORTISTRATE_YELLOW = "#F9B91C"
CORTISTRATE_YELLOW_SOFT = "#F6C23B"
CORTISTRATE_AMBER_DIM = "#4A3D20"
CORTISTRATE_AMBER = "#8B763F"
CORTISTRATE_CYAN = "#F5EDDC"
CORTISTRATE_GREEN = "#D8CDAF"
CORTISTRATE_ORANGE = "#C09525"
BRAILLE_BASE = 0x2800
BRAILLE_DOT_BITS = (
    (0x01, 0x02, 0x04, 0x40),
    (0x08, 0x10, 0x20, 0x80),
...
SPHERE_STATES: dict[str, SphereState] = {
    ...
    "source": SphereState(
        ...
        accent=CORTISTRATE_YELLOW_SOFT,
    ),
    ...
    *, width: int, height: int, phase: float, state_key: str
    ...
    for (x, y), mask in masks.items():
        ...
        if highlighted and state.key == "recalling":
            style = CORTISTRATE_CYAN
        elif highlighted:
            style = CORTISTRATE_YELLOW
        else:
...
def _style_for_depth(z: float, state: SphereState) -> str:
    if state.key == "extracting" and z > 0.38:
        return CORTISTRATE_ORANGE
    if state.key == "indexing" and z > 0.45:
        return CORTISTRATE_CYAN
    if state.key == "ingesting" and z > 0.5:
        return CORTISTRATE_CYAN
    if z > 0.58:
        return CORTISTRATE_YELLOW
    if z > 0.05:
        return CORTISTRATE_YELLOW
    return CORTISTRATE_AMBER
