...
async def inspect_artifacts(memory_root: Path) -> dict:
    """Read md files + Postgres counts after cascade drain."""
    counts = {
        "episode_rows": 0,
        "atomic_fact_rows": 0,
        "foresight_rows": 0,
...
def render_report(
    *,
    ...
    lines.append("\n## 3. Cascade drain (md → Postgres sync)\n")
    lines.append("```\n")
    ...
    # ── Section: Artifacts ────────────────────────────────────────────
    lines.append("\n## 4. On-disk artifacts\n")
    lines.append("### Postgres row counts\n\n")
...
async def main() -> None:
    ...
    async with (
        ...
        print(
            "      Postgres: {k: v for k,v in artifacts.items() if k.endswith('_rows')}"
        )
