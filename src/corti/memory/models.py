...
the ``from corti.memory import MemCell, Episode`` convenience.
"""
...
class Episode(BaseModel):
    """Domain Episode — algo-emitted business fields + corti context.

    Composed (not inherited) from :class:`everalgo.types.Episode`. corti
    ...
    corti fills it from the memcell currently being processed (the
...
class AtomicFact(BaseModel):
    """Domain AtomicFact — algo-emitted business fields + corti context.

    Composed (not inherited) from :class:`everalgo.types.AtomicFact`. Mirrors
    :class:`Episode`: corti keeps the *semantic* fields algo emits
...
class Foresight(BaseModel):
    """Domain Foresight — algo-emitted business fields + corti context.
...
class AgentCase(BaseModel):
    """Domain AgentCase — algo-emitted business fields + corti context.

    Composed (not inherited) from :class:`everalgo.types.AgentCase`. Mirrors
    :class:`Episode` / :class:`AtomicFact` / :class:`Foresight`: corti
