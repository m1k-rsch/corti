...
``cells`` feed both :class:`cortistrate.memory.extract.pipeline.UserMemoryPipeline`
and :class:`cortistrate.memory.extract.pipeline.AgentMemoryPipeline` (the
...
from cortistrate.memory import CanonicalMessage, IngestResult, ToolCall
...
def _split_messages_per_cell(
    ...
    """Map each cell index → list of cortistrate message_ids.
...
def _mint_memcell_id() -> str:
    """Generate an cortistrate-owned memcell identifier."""
