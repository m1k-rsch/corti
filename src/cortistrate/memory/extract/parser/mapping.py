...
to import without the ``cortistrate[multimodal]`` extra.

everalgo deliberately never reads the host filesystem (``http(s)`` fetch or
caller-hydrated bytes only). Cortistrate — the trust boundary that legitimately
    ...
    * confined to ``settings.multimodal.file_uri_allow_dirs`` when that
    ...
    if allow and not any(path.is_relative_to(r) for r in allow):
        raise ValueError(
            ...
            "(set CORTISTRATE_MULTIMODAL__FILE_URI_ALLOW_DIRS to permit it)"
        )
