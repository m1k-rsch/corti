...
Corti is a fork of EverOS by EverMind AI, licensed under Apache-2.0.
...
provider, redesigned for the new Corti server. The system prompt
...
scripts, and MCP server were rewritten to target the self-hosted Corti
...
Inspired the "one provider per file" flat adapter layout that Corti uses
...
Corti is built on top of excellent open-source libraries and frameworks:
