...
:mod:`cortistrate.component.rerank`.
"""

from __future__ import annotations

from cortistrate.config import RerankSettings
...
def build_rerank_provider(settings: RerankSettings) -> RerankProvider:
    ...
    if not settings.model:
        raise ValueError(
            ...
            "(set CORTISTRATE_RERANK__MODEL or [rerank] model in user toml)"
        )
    if not settings.base_url:
        raise ValueError(
            "Rerank base_url is not configured (set CORTISTRATE_RERANK__BASE_URL)"
        )
    ...
    if settings.provider == "deepinfra":
        if not api_key:
            raise ValueError(
                ...
                "(set CORTISTRATE_RERANK__API_KEY)"
            )
