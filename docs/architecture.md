...
| memory → service | ❌ |
| infra → memory | ❌ |
| infra cross-subpackage (e.g. postgres → markdown within persistence/) | ❌ (use service to orchestrate) |
...
└────────────────────────────────────────────────────────────────┘
   ...
   └──────────────┘   └──────────────┘   └─────────────────┘
          │                  │                    │
          ▼                  ▼                    ▼
    memory-root/         .index/sqlite/      .index/postgres/
...
4a. SQLite   4b. memory.cascade  (async daemon)
    audit        watches md → diff entries → Postgres sync
```

**Key guarantee**: md write is strongly consistent (fsync). Postgres is eventually consistent. Postgres unavailability does not block response — changes buffer in the SQLite `md_change_state` queue, replayed on recovery.
...
1. service.search
   │
   ▼
2. memory.search (hybrid)     single Postgres query =
...
Daemon that watches markdown changes and syncs to Postgres:

- inotify / FSEvents file watcher (cross-platform via `watchdog`)
- 500ms debounce
- Entry-level diff (added / changed / removed)
- Postgres single-transaction update (text + vector columns atomic)
