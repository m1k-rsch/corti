"""Common SQLModel base for cortistrate tables.
...
* **on write** — ``process_bind_param`` converts every datetime to
  ...
  *every* write path passes through. Mapper events (``before_insert`` /
  ...
  silently miss Core statements — which :mod:`cortistrate.infra.persistence
...
from cortistrate.component.utils.datetime import UtcDatetime, ensure_utc, get_utc_now
