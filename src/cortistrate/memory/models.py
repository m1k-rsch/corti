...
the ``from cortistrate.memory import MemCell, Episode`` convenience.
"""
...
class Episode(BaseModel):
    """Domain Episode — algo-emitted business fields + cortistrate context.

    Composed (not inherited) from :class:`everalgo.types.Episode`. cortistrate
    ...
    cortistrate fills it from the memcell currently being processed (the
    ...
    # cortistrate engineering metadata.
...
class AtomicFact(BaseModel):
    """Domain AtomicFact — algo-emitted business fields + cortistrate context.

    Composed (not inherited) from :class:`everalgo.types.AtomicFact`. Mirrors
    :class:`Episode`: cortistrate keeps the *semantic* fields algo emits
...
class Foresight(BaseModel):
    """Domain Foresight — algo-emitted business fields + cortistrate context.
...
class AgentCase(BaseModel):
    """Domain AgentCase — algo-emitted business fields + cortistrate context.

    Composed (not inherited) from :class:`everalgo.types.AgentCase`. Mirrors
    :class:`Episode` / :class:`AtomicFact` / :class:`Foresight`: cortistrate
