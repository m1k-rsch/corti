...
- ``async_client``: ``httpx.AsyncClient`` wired into ``create_app()`` with
  the full lifespan stack (SQLite + Postgres + Cascade + OME).
...
def search_seed() -> dict[str, list[dict]]:
    ...
    dicts ready to be fed into ``Model.model_validate`` for Postgres.
...
def pipeline_done_poll() -> Callable[..., Awaitable[None]]:
    ...
    chain (OME -> md -> cascade -> Postgres) must use this instead of
    ``cascade_done_poll``.
    """
