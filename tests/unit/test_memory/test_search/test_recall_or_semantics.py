"""Real-Postgres regression: OR-mode BooleanQuery sparse recall.
...
White-box surfaces:
- Postgres ``episode`` table (real, per-test tmp root)
...
async def test_or_semantics_empty_query_returns_empty() -> None:
    """Tokenisation yields nothing → recall returns ``[]`` without hitting Postgres."""
