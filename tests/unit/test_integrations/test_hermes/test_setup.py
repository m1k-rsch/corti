...
- ``write_cortistrate_toml`` emits a ``tomllib``-parseable file with
  ``[llm]`` / ``[embedding]`` (and ``[rerank]`` when provided) sections
  carrying the caller's keys, chmod 600, and omits ``[rerank]`` when None.
- ``post_setup`` in non-interactive mode writes ``cortistrate.json`` (platform)
  or ``cortistrate.toml`` + ``cortistrate.json`` for oss mode, all parseable.
"""
...
from hermes._setup import (
    build_cortistrate_json,
    post_setup,
    write_cortistrate_toml,
)
...
def test_write_cortistrate_toml_sections_keys_and_mode(tmp_path: Path) -> None:
    path = write_cortistrate_toml(
        tmp_path,
        llm={
            "model": "gpt-4",
            "api_key": "secret",
    ...
    assert path == tmp_path / "cortistrate.toml"
    assert path.exists()
...
def test_write_cortistrate_toml_omits_rerank_when_none(tmp_path: Path) -> None:
    path = write_cortistrate_toml(
        tmp_path,
        llm={"model": "m"},
        embedding={"model": "e"},
...
def test_build_cortistrate_json_returns_expected_dict() -> None:
    payload = build_cortistrate_json(
        api_url="http://api",
        mode="oss",
        user_id="u-1",
        agent_id="a-1",
        cortistrate_root="/root",
    )
    assert payload == {
        "api_url": "http://api",
        "mode": "oss",
...
def test_post_setup_platform_writes_only_cortistrate_json(tmp_path: Path) -> None:
    ...
    json_path = hermes_home / "cortistrate.json"
    assert json_path.exists()
    assert (json_path.stat().st_mode & 0o777) == _TOML_MODE
    assert json.loads(json_path.read_text(encoding="utf-8")) == payload
    assert payload["mode"] == "platform"
    assert payload["cortistrate_root"] is None
    # Platform mode does not write cortistrate.toml under hermes_home.
    assert not (hermes_home / "cortistrate.toml").exists()


def test_post_setup_oss_writes_toml_and_json(tmp_path: Path) -> None:
    hermes_home = tmp_path / "hermes"
    cortistrate_root = tmp_path / "cortistrate"
    inputs = {
        "mode": "oss",
        "cortistrate_root": str(cortistrate_root),
        "api_url": "http://api",
        "user_id": "u",
        "agent_id": "a",
    ...
    assert (hermes_home / "cortistrate.json").exists()
    assert (cortistrate_root / "cortistrate.toml").exists()

    with (cortistrate_root / "cortistrate.toml").open("rb") as fh:
        toml_data = tomllib.load(fh)
    assert toml_data["llm"]["model"] == "m"
    assert toml_data["embedding"]["model"] == "e"
    assert "rerank" not in toml_data

    json_data = json.loads((hermes_home / "cortistrate.json").read_text(encoding="utf-8"))
    assert json_data == payload
    assert json_data["mode"] == "oss"
    assert json_data["cortistrate_root"] == str(cortistrate_root)
...
def test_post_setup_oss_skips_ome_without_agent_track(tmp_path: Path) -> None:
    ...
    assert (cortistrate_root / "cortistrate.toml").exists()
    assert not (cortistrate_root / "ome.toml").exists()
    assert (hermes_home / "cortistrate.json").exists()
