...
Corti uses a two-file TOML configuration system with environment variable
overrides for container deployments.

## Quick Start

```bash
# 1. Generate config files in the default root (~/.corti/)
corti init

# 2. Edit the config — fill in API keys
$EDITOR ~/.corti/corti.toml
...
### `corti.toml` — Application Settings

Located at `<root>/corti.toml`. Controls all application behavior: API
...
Generated by `corti init` from the shipped `config/default.toml`
...
Generated by `corti init` from the shipped `config/default_ome.toml`
...
2. `<root>/corti.toml` — user configuration (optional)
3. `CORTI_*` environment variables — container/CI overrides
...
| Source | Example |
|---|---|
| `--root` CLI flag | `corti server start --root /data/corti` |
| `CORTI_ROOT` env var | `export CORTI_ROOT=/data/corti` |
| Default | `~/.corti` |

All CLI commands that interact with storage accept `--root`:

```bash
corti server start --root /data/corti
corti cascade status --root /data/corti
corti config show --root /data/corti
corti init --root /data/corti
```
...
Use `corti config show` to inspect the effective configuration:
...
In containerized environments, skip `corti init` and use environment
variables directly:

```dockerfile
ENV CORTI_ROOT=/data/corti
ENV CORTI_LLM__API_KEY=sk-...
ENV CORTI_LLM__MODEL=gpt-4o
ENV CORTI_EMBEDDING__MODEL=text-embedding-3-large
ENV CORTI_EMBEDDING__API_KEY=sk-...
ENV CORTI_EMBEDDING__BASE_URL=https://api.openai.com/v1
ENV CORTI_API__HOST=0.0.0.0
ENV CORTI_API__PORT=8000
```

Environment variable naming convention:

```
CORTI_<SECTION>__<KEY>
```
...
|---|---|
| `[llm] api_key = "sk-..."` | `CORTI_LLM__API_KEY=sk-...` |
| `[sqlite] busy_timeout_ms = 10000` | `CORTI_SQLITE__BUSY_TIMEOUT_MS=10000` |
| `[memory] timezone = "Asia/Tokyo"` | `CORTI_MEMORY__TIMEZONE=Asia/Tokyo` |
