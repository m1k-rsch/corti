...
_DEFAULT_ROOT = Path("~/.corti")


def resolve_root(explicit: str | None = None) -> Path:
    """Resolve the memory-root path.

    Priority: explicit arg > CORTI_ROOT env > ~/.corti default.
    ...
    from_env = os.environ.get("CORTI_ROOT")
    if from_env:
...
class MemorySettings(BaseModel):
    ...
    TOML or ``CORTI_MEMORY__TIMEZONE`` env var. Validated against
...
class ApiSettings(BaseModel):
    ...
    matching the threat model in ``SECURITY.md``: Corti ships **no
...
class MultimodalSettings(BaseModel):
    ...
    Env binding (via parent ``Settings``):
        CORTI_MULTIMODAL__MODEL
        CORTI_MULTIMODAL__API_KEY
        CORTI_MULTIMODAL__BASE_URL
        CORTI_MULTIMODAL__MAX_CONCURRENCY
        CORTI_MULTIMODAL__FILE_URI_ALLOW_DIRS
        CORTI_MULTIMODAL__FILE_URI_MAX_BYTES
    """
...
class EmbeddingSettings(BaseModel):
    ...
    Env binding:
        CORTI_EMBEDDING__MODEL
        CORTI_EMBEDDING__API_KEY
        CORTI_EMBEDDING__BASE_URL
        CORTI_EMBEDDING__TIMEOUT_SECONDS
        CORTI_EMBEDDING__MAX_RETRIES
        CORTI_EMBEDDING__BATCH_SIZE
        CORTI_EMBEDDING__MAX_CONCURRENT
    """
...
class RerankSettings(BaseModel):
    ...
    Env binding:
        CORTI_RERANK__PROVIDER
        CORTI_RERANK__MODEL
        CORTI_RERANK__API_KEY
        CORTI_RERANK__BASE_URL
        CORTI_RERANK__TIMEOUT_SECONDS
        CORTI_RERANK__MAX_RETRIES
        CORTI_RERANK__BATCH_SIZE
        CORTI_RERANK__MAX_CONCURRENT
    """
...
class MemorizeSettings(BaseModel):
    ...
    Env binding:
        CORTI_MEMORIZE__MODE
        CORTI_MEMORIZE__SESSION_LOCK_TIMEOUT_SECONDS
    """
...
class ClusteringSettings(BaseModel):
    ...
    Env binding:
        CORTI_CLUSTERING__THRESHOLD
        CORTI_CLUSTERING__TIME_WINDOW_DAYS
    """
...
class Settings(BaseSettings):
    ...
    def __init__(self, *, _corti_root: Path | None = None, **kwargs: object) -> None:
        """Initialise settings, optionally pinning the memory-root for testing.

        Args:
            _corti_root: Override the memory root used to locate
                ``corti.toml``. Intended for tests only; pass ``None``
                (the default) in production to use :func:`resolve_root`.
            **kwargs: Forwarded verbatim to :class:`pydantic_settings.BaseSettings`.
        """
        if _corti_root is not None:
            # Temporarily inject CORTI_ROOT so that settings_customise_sources
            ...
            _prev = os.environ.get("CORTI_ROOT")
            os.environ["CORTI_ROOT"] = str(_corti_root)
            try:
                super().__init__(**kwargs)
            finally:
                if _prev is None:
                    os.environ.pop("CORTI_ROOT", None)
                else:
                    os.environ["CORTI_ROOT"] = _prev
        else:
            super().__init__(**kwargs)
    ...
    def settings_customise_sources(
        ...
        """Source order: init_args > env_vars > corti.toml > default.toml."""
