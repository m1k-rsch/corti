...
lint:
	...
	uv run python src/scripts/check_github_contributor_docs.py
	uv run python src/scripts/check_datetime_discipline.py
	uv run python src/scripts/dump_openapi.py --check

docs-check:
	python3 src/scripts/check_docs.py
	python3 src/scripts/check_github_contributor_docs.py
	ruby -e 'require "yaml"; Dir[".github/ISSUE_TEMPLATE/*.yml"].sort.each { |p| YAML.load_file(p); puts "YAML ok: #{p}" }'

check-commits:
	python3 src/scripts/check_commit_messages.py $(RANGE)

check-pr-title:
	python3 src/scripts/check_pr_title.py
...
check-cjk:
	uv run python src/scripts/check_cjk.py
...
openapi:
	uv run python src/scripts/dump_openapi.py
