"""Real-Postgres tests for ``ProfileRecaller`` — KV-by-owner fetch.
...
the Postgres path (no stubs) and the JSON unpacking that turns the
...
async def test_fetch_returns_empty_for_blank_owner() -> None:
    """Blank ``owner_id`` short-circuits — never hit Postgres with an
