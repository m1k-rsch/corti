...
The default location and tunables come from :class:`corti.config.Settings`
(loaded from ``config/default.toml`` + ``CORTI_*`` environment variables);
...
class MemoryRoot:
    ...
    def default(cls, *, explicit_root: str | None = None) -> MemoryRoot:
        ...
        Resolution: ``explicit_root`` > ``CORTI_ROOT`` env > ``~/.corti``.
