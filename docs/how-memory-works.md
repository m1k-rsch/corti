...
How Corti turns a stream of messages into durable, searchable memory —
...
The default memory root is **`~/.corti/`** (override with
`CORTI_ROOT` env var or `--root` on the CLI). Configuration lives
inside the memory root as `corti.toml` (generated by `corti init`).
...
!!! warning "Differences from older PRD-era docs"
    ...
    scope). There is **no `corti reindex` command** (see
    [Operating it](#operating-it)).

The path manager is
[`MemoryRoot`](../src/corti/core/persistence/memory_root.py); every path
...
write. Config files (`corti.toml`, `ome.toml`) are created by `corti init`.
...
with `corti cascade` ([Operating it](#operating-it)); deeper runbook in
...
|---|---|
| `corti init` | generate starter config files (`corti.toml` + `ome.toml`) |
| `corti server start` | run the HTTP API (cascade + OME start with it) |
| `corti cascade status` | queue / LSN summary |
| `corti cascade sync` | drain the cascade queue now (force md → Postgres) |
| `corti cascade fix` | list failed rows / re-enqueue retryable ones |

!!! warning "There is no `corti reindex` or `corti flush`"
