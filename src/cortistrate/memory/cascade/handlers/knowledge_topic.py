"""KnowledgeTopic cascade handler — md → Postgres + SQLite.

Unlike every other handler which writes to Postgres only, the knowledge
topic handler writes to **both** Postgres (summary embedding + BM25
...
from cortistrate.core.persistence import MarkdownReader, ParsedMarkdown
from cortistrate.infra.persistence.pg import KnowledgeTopic, knowledge_topic_repo
...
class KnowledgeTopicHandler(Handler):
    ...
    db_repo: ClassVar[Any] = knowledge_topic_repo
    ...
    async def handle_added_or_modified(self, md_path: str) -> HandlerOutcome:
        ...
        row = await self._build_db_row(fields, digest, md_path)
    ...
    async def _build_db_row(
        self,
        fields: dict[str, Any],
        digest: str,
        md_path: str,
    ) -> KnowledgeTopic:
        """Tokenize, embed, and construct the Postgres row."""
        summary_tokens = " ".join(
    ...
    async def handle_deleted(self, md_path: str) -> HandlerOutcome:
        pg_deleted = await knowledge_topic_repo.delete_by_md_path(md_path)
        sqlite_deleted = await knowledge_topic_sqlite_repo.delete_by_md_path(md_path)
        deleted = max(pg_deleted, sqlite_deleted)
        return HandlerOutcome(
            md_path=md_path,
            kind=self.kind,
            upserted=0,
