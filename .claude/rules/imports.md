...
- **Import order** (ruff `I` enforces, `make format` fixes): stdlib → third-party
  → first-party (`everalgo`, then `cortistrate`). One group per blank-line-separated block.
- **Absolute imports** for cross-package references (`from cortistrate.memory import ...`).
