...
single source of truth; SQLite and Postgres are derived indexes built
...
```
Markdown (source of truth)  +  SQLite (structured state)  +  Postgres (vector + BM25 index)
```
...
| Postgres | Topic vectors, BM25 tokens, scalar fields | Search index (fully rebuildable from Markdown) |

Even if SQLite and Postgres data is corrupted, as long as the Markdown
...
2. **Recall** — dual-channel retrieval from Postgres:
...
changes and keeps SQLite + Postgres in sync.
...
md file written
  ...
  → handler dispatched by file type:
      ...
      N_topic.md  → KnowledgeTopicHandler    → tokenize + embed + SQLite + Postgres upsert
```
...
Storage paths, SQLite rows, and Postgres indexes are all scoped by
`app_id` + `project_id`.
