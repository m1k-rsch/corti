"""Strict md ↔ Postgres consistency check for all cascade kinds.
...
- **id set** — md entry id (or single-row PK) == Postgres row id.
- **content** — md-recomputed ``content_sha256`` ==
  Postgres row ``content_sha256``.
...
exercises every kind that writes md and indexes into Postgres, not just
the 4 daily-log kinds covered by the white-box integration test.

Daily-log kinds (atomic_fact / episode / foresight) hold
...
async def assert_md_lance_strict_consistent(
    ...
    *,
    ...
    """For every kind in :data:`KIND_REGISTRY`, assert md ↔ Postgres parity.
...
def _q(text: str) -> str:
    """SQL-quote escape; mirrors Postgres chassis convention."""
    return text.replace("'", "''")
