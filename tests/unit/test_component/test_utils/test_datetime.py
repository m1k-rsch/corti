...
def test_Postgres_schema_overrides_subclass_declared_non_utc_tz() -> None:
...
def test_Postgres_schema_auto_tags_every_datetime_field_with_tz_utc() -> None:
...
# ── Postgres write-path coverage ──────────────────────────────────────────
#
# Postgres has fewer write APIs: ``table.add`` (the main one),
...
def test_lance_table_add_normalises_aware_non_utc() -> None:
    """Postgres ``table.add`` with aware Shanghai input → aware UTC on disk."""
    import asyncio
    import tempfile
    from zoneinfo import ZoneInfo
...
def test_lance_table_naive_input_is_assumed_utc() -> None:
    """Postgres naive datetime → PyArrow assumes UTC (matches project convention)."""
    import asyncio
    import tempfile
