...
and the originals are deprecated in both md frontmatter and Postgres.

See ``local/2026-06-14-reflection-cortistrate-design.md`` for the full design.
"""
...
from cortistrate.core.errors import AppError
...
from cortistrate.memory._partition_locks import get_partition_lock
...
def _escape_sql(value: str) -> str:
    """Escape single quotes for Postgres SQL ``WHERE`` predicates.

    Postgres uses parameterised queries; doubling the quote
...
class ReflectionOrchestrator:
    ...
    Args:
        ...
        episode_store: Postgres episode repository (read + update).
        atomic_fact_store: Postgres atomic fact repository (update).
    ...
    async def _reflect_cluster(
        ...
        *,
        ...
        Args:
            episodes: Source episode rows from Postgres.
    ...
    async def _load_cluster_episodes(
        ...
        *,
        ...
        """Read cluster members and fetch their episode rows from Postgres.
    ...
    async def _apply_deprecation_writes(
        ...
        *,
        ...
        """Patch md frontmatter and mark episodes/facts deprecated in Postgres.
        ...
        deprecated_ep_count = await self._deprecate_db_episodes(
        ...
        deprecated_fact_count = await self._deprecate_db_facts(
    ...
    async def _deprecate_db_episodes(
        self,
        *,
        entry_ids: set[str],
        owner_id: str,
        ...
        """Mark deprecated episodes in Postgres by entry_id.

        Returns:
            Number of Postgres update calls issued.
        """
        coros: list[Any] = [
    ...
    async def _deprecate_db_facts(
        ...
        *,
        ...
        """Mark deprecated atomic facts in Postgres.
        ...
        Returns:
            Total number of Postgres update calls issued.
        """
        if not parent_ids:
            return 0
    ...
    async def _create_reflection_report(
        ...
        *,
        ...
        from cortistrate.infra.persistence.sqlite import ReflectionReport
...
def _to_algo_episodes(episodes: list[Any]) -> list[AlgoEpisode]:
    """Convert Postgres episode rows to algo Episode objects.
...
def _ts_to_ms(ts: object) -> int:
    ...
    Postgres episode rows store ``timestamp`` as a ``datetime`` object;
