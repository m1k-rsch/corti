"""``cortistrate integrations`` — install Cortistrate bundles into agent runtimes.
...
``~/.cortistrate/.integrations-sentinel``) on first ``cortistrate``
...
app = typer.Typer(
    name="integrations",
    help="Install Cortistrate integrations into agent runtimes.",
...
_HERMES_PLUGIN_SUBDIR = Path("plugins") / "cortistrate"
_CLAUDE_SKILLS_SUBDIR = Path("skills") / "cortistrate"

# Sentinel file — written after first auto-detect so we never scan twice.
_SENTINEL = Path.home() / ".cortistrate" / ".integrations-sentinel"
...
def _resolve_bundle_root() -> Path | None:
    ...
    2. Walk up from ``cortistrate.__file__`` → ``src/integrations/`` (dev checkout)
    """

    # Pip-installed: the integrations namespace lives next to cortistrate.
    try:
        import cortistrate
    except ImportError:
        cortistrate = None  # type: ignore[assignment]
    ...
    # Dev checkout: walk up from cortistrate's location.
    if cortistrate is not None:
        try:
            here = Path(cortistrate.__file__).resolve()
...
def _resolve_target_bundle(target: str, explicit: str | None = None) -> Path:
    ...
    if bundle_root is None:
        raise typer.BadParameter(
            ...
            "Ensure cortistrate is installed via pip, or pass --source PATH."
        )
...
def _install_hermes(bundle_src: Path, force: bool = False) -> Path:
    """Symlink ``~/.hermes/plugins/cortistrate`` → ``bundle_src``."""
...
def _install_claude(bundle_src: Path, force: bool = False) -> Path:
    """Symlink ``~/.claude/skills/cortistrate`` → ``bundle_src``."""
    skills_dir = _resolve_claude_skills_dir()
    skills_dir.mkdir(parents=True, exist_ok=True)
    target_path = skills_dir / "cortistrate"
    _do_symlink(target_path, bundle_src, force)
    return target_path
...
def _is_installed(target: str) -> bool:
    ...
    if target == _HERMES_TARGET:
        ...
        link = _resolve_claude_skills_dir() / "cortistrate"
...
def install(
    ...
    """Symlink the Cortistrate integration into an agent's plugin directory."""
    ...
    if target == _HERMES_TARGET:
        ...
        typer.echo(
            "\nNext:\n"
            "  1. Run `hermes memory setup` and select 'cortistrate'.\n"
            "  2. Verify: `hermes cortistrate status`."
        )
    else:
        ...
        typer.echo(
            "\nNext:\n"
            "  Start Claude Code. The plugin loads as cortistrate@skills-dir.\n"
            "  Verify: run `/cortistrate:mem-search \"test\"` in Claude Code."
        )
...
def uninstall(
    ...
    force: bool = typer.Option(
        False,
        "--force",
        help="Unlink even if the symlink doesn't point to a cortistrate bundle.",
    ),
) -> None:
    """Remove the Cortistrate integration symlink."""
    if target not in _SUPPORTED:
        raise typer.BadParameter(
            f"Unsupported target: {target!r}. "
            f"Supported: {', '.join(sorted(_SUPPORTED))}."
    ...
    if not force:
        # Quick sanity: does the symlink point at something that looks like cortistrate?
        resolved = link.resolve()
        if not (resolved / "plugin.yaml").exists() and not (
            resolved / ".claude-plugin" / "plugin.json"
        ).exists():
            msg = (
                f"{link} does not appear to point to a cortistrate "
...
def status() -> None:
    ...
    typer.echo("Cortistrate integrations\n")
...
def auto_detect_and_install(
    ...
    pkg = Path(cortistrate.__file__).resolve()
    in_site = any(
...
def _install_silent(target: str, bundle_root: Path) -> None:
    ...
    logger.info(
        "cortistrate.integration.auto_installed",
        target=target,
