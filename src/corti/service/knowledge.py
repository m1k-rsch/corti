...
from corti.core.persistence.markdown import dump_frontmatter, parse_frontmatter
...
if TYPE_CHECKING:
    ...
    from corti.config.settings import KnowledgeSearchSettings
...
def _get_embedding() -> EmbeddingProvider | None:
    ...
    from corti.component.embedding import (  # Deferred: singleton
        build_embedding_provider,
    )
    from corti.config import load_settings  # Deferred: singleton
...
def _build_recaller() -> KnowledgeTopicRecaller:
    ...
    from corti.component.tokenizer import (  # Deferred: singleton
        build_tokenizer,
    )
    from corti.memory.search.recall import (  # Deferred: singleton
...
def _get_reranker() -> RerankProvider | None:
    ...
    from corti.component.rerank import (  # Deferred: singleton
...
def _require_search_providers() -> tuple[EmbeddingProvider, RerankProvider]:
    ...
    if embedder is None:
        raise ConfigurationError(
            ...
            "Set CORTI_EMBEDDING__MODEL and CORTI_EMBEDDING__API_KEY."
        )
    reranker = _get_reranker()
    if reranker is None:
        raise ConfigurationError(
            "Rerank provider not configured. "
            "Set CORTI_RERANK__MODEL and CORTI_RERANK__BASE_URL."
        )
...
async def _run_category_pipeline(
    ...
    *,
    ...
    from corti.memory.search.callbacks import (  # Deferred: heavy dep
        build_rerank_fn,
    )
