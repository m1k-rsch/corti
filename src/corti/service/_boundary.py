...
``cells`` feed both :class:`corti.memory.extract.pipeline.UserMemoryPipeline`
and :class:`corti.memory.extract.pipeline.AgentMemoryPipeline` (the
...
from corti.memory import CanonicalMessage, IngestResult, ToolCall
...
def _split_messages_per_cell(
    ...
    """Map each cell index → list of corti message_ids.
...
def _mint_memcell_id() -> str:
    """Generate an corti-owned memcell identifier."""
