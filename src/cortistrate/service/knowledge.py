...
Functions:
    ...
    delete_document   — remove document directory (cascade handles SQLite/Postgres).
...
from cortistrate.core.persistence.markdown import dump_frontmatter, parse_frontmatter
...
if TYPE_CHECKING:
    ...
    from cortistrate.config.settings import KnowledgeSearchSettings
...
async def delete_document(
    ...
    """Remove a document directory; cascade handles SQLite/Postgres cleanup.
...
def _get_embedding() -> EmbeddingProvider | None:
    ...
    from cortistrate.component.embedding import (  # Deferred: singleton
        build_embedding_provider,
    )
    from cortistrate.config import load_settings  # Deferred: singleton
...
def _build_recaller() -> KnowledgeTopicRecaller:
    ...
    from cortistrate.component.tokenizer import (  # Deferred: singleton
        build_tokenizer,
    )
    from cortistrate.memory.search.recall import (  # Deferred: singleton
...
def _get_reranker() -> RerankProvider | None:
    ...
    from cortistrate.component.rerank import (  # Deferred: singleton
...
def _validate_scope_id(value: str, name: str) -> None:
    """Reject scope ids with characters that could break Postgres SQL.
...
def compile_knowledge_where(app_id: str, project_id: str) -> str:
    """Build a Postgres ``WHERE`` clause scoped to the given tenant.
    ...
    Returns:
        SQL-style predicate string safe for use in Postgres ``WHERE`` parameter.
...
def _require_search_providers() -> tuple[EmbeddingProvider, RerankProvider]:
    ...
    if embedder is None:
        raise ConfigurationError(
            ...
            "Set CORTISTRATE_EMBEDDING__MODEL and CORTISTRATE_EMBEDDING__API_KEY."
        )
    reranker = _get_reranker()
    if reranker is None:
        raise ConfigurationError(
            "Rerank provider not configured. "
            "Set CORTISTRATE_RERANK__MODEL and CORTISTRATE_RERANK__BASE_URL."
        )
...
async def _run_category_pipeline(
    ...
    *,
    ...
    from cortistrate.memory.search.callbacks import (  # Deferred: heavy dep
        build_rerank_fn,
    )
